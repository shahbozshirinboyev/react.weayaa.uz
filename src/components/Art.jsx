  ...
  return (
    <section ref={ref} className="container p-4 pt-0 pb-[25px] select-none">
        ...
        >
          {galleryInfo.map((gallery) => (
            <SwiperSlide key={gallery.id}>
              <button
                onClick={() => {
                  filterTab(gallery.counts); setActiveTab(gallery.id)
                }}
                className={`border-transparent font-semibold ${ activeTab === gallery.id ? "text-white bg-mainColor" : "text-mainColor bg-mainColor bg-opacity-30"} transition-all duration-300  px-2 py-1 rounded-lg text-center text`}
              >
                {gallery.name}
              </button>
            </SwiperSlide>
      ...
      <div className="columns-2 lg:columns-3 xl:columns-4 py-8">
          {images.map((item) => (
            <div key={item.id} className="m-2 transition-all duration-300 cursor-pointer">
